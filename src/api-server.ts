...
import { sendMessage, sendMedia, getMessages, forwardMessage, deleteMessage } from './tools/messaging.js';
import { getChats, getChatInfo, archiveChat, pinChat, muteChat, markAsRead, clearMessages, deleteChat } from './tools/chats.js';
import { getContacts, getContactInfo, blockContact, getProfilePic, getContactAbout, getCommonGroups, searchContacts } from './tools/contacts.js';
...
function handleApiResponse(result: any, res: express.Response) {
  if (result.isError) {
 ...
 */
app.post('/whatsapp/send-message', async (req: express.Request, res: express.Response) => {
  try {
 ...
 *         description: Invalid request or WhatsApp error
 */
app.post('/whatsapp/send-media', async (req: express.Request, res: express.Response) => {
  try {
 ...
 *         description: Invalid request or WhatsApp error
 */
app.get('/whatsapp/get-messages', async (req: express.Request, res: express.Response) => {
  try {
 ...
 *         description: Invalid request or WhatsApp error
 */
app.get('/whatsapp/chat-info', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const result = await getChatInfo({ chatId: chatId as string });
    handleApiResponse(result, res);
  } catch (error) {
    res.status(500).json({
      error: 'Internal server error',
 ...
 *         description: Invalid request or WhatsApp error
 */
app.get('/whatsapp/get-contacts', async (req: express.Request, res: express.Response) => {
  try {
 ...
 *         description: Invalid request or WhatsApp error
 */
app.get('/whatsapp/search-contacts', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const result = await searchContacts({ query: query as string });
    handleApiResponse(result, res);
  } catch (error) {
    res.status(500).json({
      error: 'Internal server error',
...
app.post('/whatsapp/click-chat', async (req, res) => {
  try {
    ...
    const result = await whatsAppConnection.client.pupPage.evaluate((index: number, name: string) => {
      try {
        // Find chat elements
